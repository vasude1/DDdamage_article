...
\subsection*{In 2D}
The generating model is $\sigma^*_i = g(D) E_{ij} \epsilon^*_j$. $g$ is the degradation function of the damage variable, $D$. The damage evolution in the present case is taken to be a function of strain as 
\begin{equation}
...
\begin{equation}
	\parder{d}{\epsilon^*_k} = -\parder{\sigma^*_i}{\epsilon^*_k} S_{ij} (\sig - \sigma^*)_j - \parder{\epsilon^*_i}{\epsilon^*_k} C_{ij}(\eps-\epsilon^*)_j.
\end{equation}
Realizing that $\sigma^*_i = g(d) E_{ij} \eps^*_j$, the derivative can be written as 
\begin{gather}
	\parder{\sigma^*_i}{\eps^*_k} = g'(D) \parder{D}{\eps^*_k} E_{ij} \eps^*_j + g(D) E_{ij} \delta_{jk}, \\
	= -\frac{\sqrt{k Y_c}}{2(k-1)\psi_0 \sqrt{\psi_0}} \sigma^*hat_i \sigma^*hat_k + g(D) E_{ik}.
\end{gather}
Thus, the equation to be resolved can be seen to be
\begin{equation}
	r_k = \left(\frac{\sqrt{k Y_c}}{2(k-1)\psi_0 \sqrt{\psi_0}} \hat{\sigma}^*_i \hat{\sigma}^*_k + g(D) E_{ik} \right) S_{ij} (\sig - \sigma^*)_j - \parder{\epsilon^*_i}{\epsilon^*_k} C_{ij}(\eps-\epsilon^*)_j = 0.
\end{equation}
...
\begin{multline}
	\parder{r_k}{\epsilon^*_p} = \left\{  \frac{\sqrt{k Y_c}}{2(k-1)} \left[ \frac{ E_{kp} \hat{\sigma}^*_i + \hat{\sigma}^*_k E_{ip} + \hat{\sigma}^*_p E_{ik} }{\psi_0 \sqrt{\psi_0}}-\frac{3}{2} \psi_0^{-5/2} \hat{\sigma}^*_i \hat{\sigma}^*_k \hat{\sigma}^*_p \right] S_{ij} (\sig - \sigma^*)_j  \right\} +  \\ 
	\left(\frac{\sqrt{k Y_c}}{2(k-1)\psi_0 \sqrt{\psi_0}} \hat{\sigma}^*_i \hat{\sigma}^*_k + g(D) E_{ik} \right) S_{ij} \left(\frac{\sqrt{k Y_c}}{2(k-1)\psi_0 \sqrt{\psi_0}} \hat{\sigma}^*_j \hat{\sigma}^*_p + g(D) E_{jp} \right) + C_{kp}.
\end{multline}
