...
\end{multline}
where $g$ is a function such that $g(0)=1$ and $g(1)=0$. The function $\psi$ from equation \eqref{mindlin_energy} can be written in discrete form over a dual element $i$ as
\begin{equation}
...
The material parameters are the same as the previous example except $\ell_c = \SI{1}{\meter}$ \LS{(larger than beam size?)}. A crack is expected to initiate at the inner corner of the L-beam due to the pronounced stress concentration in this region.
...
This research was supported by Nantes Université and ISite NExT, through funding of IRP project iDDrEAM.
