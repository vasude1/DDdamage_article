...
\end{equation}
where $\bfw$ is vector containing the areas of elements, and $\mathcal{C}$ is a hyper-parameter. $\bfeps,\bfsig,\bfeps^*, \bfsig^* \in \mathbb{R}^{3N}$ in 2D, $N$ denotes the number of elements, and $\bfw \in \mathbb{R}^{3N}$. $\bfw \bfsig$ thus indicates an element-wise operation. The discrete equilibrium manifold is defined as  
\begin{equation}
...
The mechanical update is left as is. The overall algorithm has been presented in Algorithm \ref{alg:the_dd_algorithm}, explained later.
...
\color{red}
Before discussing the results, it is important to delimit the range of validity of the spectral analysis of Section \ref{conv_method_rate}. The contraction estimate based on the spectral radius of $\bfA\bfC$ presumes that the iteration coincides with the affine map $\bfa^{i+1} = \bfA\bfC\,\bfa^{i} + \bff$. This holds only while every material projection returns a state lying on the corresponding branch and is admissible, so that the closed-form updates of Appendix \ref{appx_convergence} apply unmodified. It is possible that the intermediate material states obtained from equation \ref{mat_update_short} are inadmissible - that is, they may either be such that the resulting stress state $\sigst$ is larger than the peak stress or that the strain state $\epsst$ for the damage branch is smaller than the strain from the previous time step. The physical database, however, carries no stress above the peak $\sigma_{\mathrm{peak}}$, and, by irreversibility, the material state of a damaged element cannot retreat to a damage level below that already attained. When either bound becomes active, the material update is no longer the linear operator $\bfA$ but $\bfA$ composed with a projection onto the admissible segment. In that regime the iterated map is no longer $\bfA\bfC$, and its spectral radius, however computed for the interior map, no longer governs the behavior of the iteration. This is precisely what occurs under displacement control beyond the snapback threshold. Two distinct constraint-saturation mechanisms arise, depending on the state from which the load step is initiated.
