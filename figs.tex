...
\end{equation}
In case where the material behavior is defined by a discrete collection of stress-strain pairs, a strict intersection may not exist between the two manifolds. In such a case, the solution is sought as the mechanical-material states that are closest to each other with respect to a certain distance. In discrete case, the distance is defined as  
\begin{equation}
	d(\bfeps,\bfsig,\bfeps^*,\bfsig^*) = \frac{1}{2}(\bfeps - \bfeps^*)^T\mathcal{C} \bfw(\bfeps - \bfeps^*) + \frac{1}{2}(\bfsig - \bfsig^*)^T\mathcal{C}^{-1}\bfw(\bfsig - \bfsig^*),
\end{equation}
The discrete equilibrium manifold is defined as  
\begin{equation}
...
As mentioned in the previous section, the strain based Lipschitz constraint introduces a length scale into the problem and renders the solution mesh independent. However, as was seen, the inequality constraint introduces a saw-tooth like pattern in the force-displacement response. This, combined with the arc-length method, would render the method tedious and expensive?? Thus, two modifications will be introduced in this section in its extension to 2D. First, the constraint will be gradually weakened as the damage level increases. Second, the inequality constraint will be replaced by a log-barrier-like function. The EL equations corresponding to the regularization technique will be derived in the appendix.
...
\begin{equation}
	\Pi  = d -\bfeta ^T \bfB^T \bfw \bfsig - \mu \bfw' \ln \bfs_u + \mu \bfw' \ln \bfs_\eta + \bflam_u\left\{ \bfs_u  - \left[ \frac{1}{\ell_c}- \psi(\bfu) \right]  \right\} 
	+ \bflam_\eta \left\{ \bfs_\eta  - \left[ \frac{1}{\ell_c}- \psi(\bfu+\bfeta) \right]  \right\}.
\end{equation}

When the constraints are to be relaxed gradually, the functional becomes
\begin{equation}
	\Pi  = d -\bfeta ^T \bfB^T \bfw \bfsig - \mu \bfw' \ln \bfs_u + \mu \bfw' \ln \bfs_\eta + \bflam_u\left\{ \bfs_u  - \left[ \frac{1}{\ell_c}- \bfg(\bfd)\psi(\bfu) \right]  \right\} 
	+ \bflam_\eta \left\{ \bfs_\eta  - \left[ \frac{1}{\ell_c}- \bfg(\bfd)\psi(\bfu+\bfeta) \right]  \right\},
\end{equation}
where $\bfg$ is a function such that $g(0)=1$ and $g(1)=0$.
...
Now, the Euler Lagrange equations can be obtained by taking the variations of the above potential energy functional.
\begin{gather}
	content...
\end{gather}
